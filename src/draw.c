...
void draw_circuits(client_t *c) {
  ...
  static double dt;
  static char pbuf[256], progress[32], *phead;
  int i, k;
  WINDOW *cw;
  circuit_t *circuit;
...
void draw_input(client_t *c) {
  ...
  sprintf(c->print_buf, "-=  %s  =-", isbset(c->s_flags, S_POLL) ? "INPUT DISABLED" : "INPUT ACTIVE");
	center_str(c->cmd, 1, c->print_buf);
	wclrtorng(c->cmd, 3, 1, CWIN_WIDTH - 4);
  prompt = isbset(c->s_flags, S_POLL) ? '!' : '>';
  if (c->cursor > CWIN_WIDTH - 6) {
    mvwprintw(c->cmd, 3, 1, "%c %s", prompt, c->cmd_input + (c->cursor - CWIN_WIDTH + 6));
  }	else {
...
void draw_packet(client_t *c, packet_t *p) {
  ...
  print += sprintf(print, "PACKET TYPE: %s\n", pktype_map[p->packet.type]);
	print += sprintf(print, "PACKET FLAGS (%u):", p->packet.flags);
  print += mapflag(p->packet.flags, 8, flag_map[p->packet.type], print);
  print += sprintf(print, "\nPACKET TIMEOUT: %d\n", p->packet.timeout);
